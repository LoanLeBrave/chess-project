...
class RobotCalibration:
    ...
    def mode_descente_interactive(self):
        ...
        print("     ↓ ou S : Descendre de 1mm")
        print("     ↑ ou W : Remonter de 1mm")
        ...
        try:
            ...
            while True:
                ...
                if position_actuelle is None:
                    position_actuelle = list(pose)
                
                # Afficher la position toutes les 0.5s
                ...
                if key:
                    # Désactiver freedrive pour mouvement contrôlé
                    if en_freedrive:
                    ...
                    # Traiter la touche
                    if key.lower() == 's' or key == '\x1b':  # 's' ou flèche
                        # Gérer les séquences d'échappement pour les flèches
                        if key == '\x1b':
                        ...
                        if key.lower() == 's':
                            # Descendre
                            position_actuelle[2] -= self.pas_descente
                            self.rtde_c.moveL(position_actuelle, self.vitesse_fine, ACCELERATION, asynchronous=False)
                            print(f"\n⬇️  Descente de {self.pas_descente*1000:.1f}mm")
                    
                    elif key.lower() == 'w':  # 'w' ou flèche haut
                        # Remonter
                        position_actuelle[2] += self.pas_descente
                        self.rtde_c.moveL(position_actuelle, self.vitesse_fine, ACCELERATION, asynchronous=False)
                        print(f"\n⬆️  Montée de {self.pas_descente*1000:.1f}mm")
                    ...
                    # Aucune touche pressée -> réactiver freedrive
                    if not en_freedrive:
                        time.sleep(0.1)  # Petit délai avant réactivation
                        position_actuelle = list(self.rtde_r.getActualTCPPose())
                        self.enable_freedrive_xy()
                        en_freedrive = True
                
                time.sleep(0.05)  # 50ms entre lectures
    ...
    def calculer_offset(self, position_mesuree):
        ...
        print("="*60)
        print(f"Position théorique: X={position_theorique[0]:.4f}, Y={position_theorique[1]:.4f}, Z={position_theorique[2]:.4f}")
        print(f"Position mesurée:   X={position_mesuree[0]:.4f}, Y={position_mesuree[1]:.4f}, Z={position_mesuree[2]:.4f}")
        print(f"\nOffset calculé:")
