...
├── calibration.py         # Module de calibration du robot
└── CALIBRATION.md         # Guide de calibration détaillé
```
...
### `calibration.py`
Module de calibration du robot :
- Utilise un trou percé dans l'échiquier comme référence
- Mode freedrive X/Y pour ajustement manuel
- Descente contrôlée par clavier (1mm par appui)
- Calcul automatique de l'offset
- Application de l'offset à toutes les positions
...
Avant la première utilisation ou après avoir déplacé l'échiquier :
```bash
python calibration.py
```
Voir [CALIBRATION.md](CALIBRATION.md) pour le guide détaillé.
