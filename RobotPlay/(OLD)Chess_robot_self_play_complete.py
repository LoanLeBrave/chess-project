#!/usr/bin/env python3
"""
Script de jeu d'échecs autonome COMPLET - Robot UR5e joue contre lui-même
Version avec contrôle robot intégré (utilise chess_robot_control.py)
"""
...
# Import du module de contrôle robot
try:
    ...
    try:
        ...
        from chess_robot_control import ChessRobotPlayer

        ROBOT_AVAILABLE = True
    except ImportError:
        ROBOT_AVAILABLE = False
        print("⚠ Module ControleRobotV2 non trouvé - Mode simulation uniquement")
...
class ChessRobotSelfPlayComplete:
    ...
    def __init__(self, stockfish_path=None, mapping_file="chess_board_positions.json",
        ...
        # Robot
        if not simulate_robot and ROBOT_AVAILABLE:
            try:
                self.robot = ChessRobotPlayer(mapping_file)
                print("✓ Robot initialisé")
            except Exception as e:
                print(f"⚠ Robot non disponible: {e}")
                self.simulate_robot = True
        else:
            self.simulate_robot = True
            if not ROBOT_AVAILABLE:
                print("⚠ Module robot non disponible - Simulation")
    ...
    def _find_stockfish(self, path=None):
        ...
        for p in paths:
            if p and (Path(p).exists() or p == 'stockfish'):
                return p
        return None
    ...
    def get_best_move(self, player):
        ...
        if not self.engine:
            ...
            return (random.choice(legal), {'evaluation': 0.0}) if legal else (None, {'error': 'No moves'})
    ...
    def execute_move_on_robot(self, move):
        ...
        try:
            if is_capture:
                ...
                return self.robot.capturer_piece(from_sq, to_sq)
            else:
                print(f"   🤖 Déplacement: {from_sq} → {to_sq}")
                return self.robot.deplacer_piece(from_sq, to_sq)
        except Exception as e:
    ...
    def play_game(self, player_white, player_black, max_moves=200, delay=1.0,
        ...
        Args:
            ...
            max_moves: Limite de coups
            delay: Délai entre coups (s)
            pause_between_moves: Pause manuelle entre chaque coup
        """
        print("\n" + "=" * 70)
        ...
        while not self.board.is_game_over() and move_num < max_moves:
            ...
            if not self.execute_move_on_robot(best_move):
                print("⚠ Erreur robot - Partie arrêtée")
                break

            # Jouer sur l'échiquier
...
def main():
    parser = argparse.ArgumentParser(description="Robot échecs autonome - Partie complète")
    ...
    parser.add_argument('--max-coups', type=int, default=100, help='Limite de coups')
    parser.add_argument('--delai', type=float, default=0.5, help='Délai entre coups (s)')
    ...
    print("=" * 70)
    print("     ♔ ROBOT ÉCHECS - JEU AUTONOME COMPLET ♚")
    print("=" * 70)
    ...
    # Jeu
    game = ChessRobotSelfPlayComplete(
